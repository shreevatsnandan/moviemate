...
export function MovieBanner({ movie_id, className, movie_title }: MovieBannerProps) {
  ...
  useEffect(() => {
    const fetchBanner = async () => {
      try {
        setIsLoading(true);
        // Replace this with your actual API call to get movie banner
        // This is a mock implementation using TMDB API as example
  ...
  return (
    <div className={cn("relative w-full h-[60vh] overflow-hidden", className)}>
      <Image
        src={bannerUrl || "/images/banner1.jpg"} // Fallback to local image if bannerUrl is null
        alt="Movie banner"
        fill
      ...
      {/* Movie title displayed on banner */}
    
        <div className="absolute bottom-0 left-0 p-8 pl-20 text-white">
          <h1 className="text-4xl font-bold drop-shadow-lg">{movie_title || "Squid Game"}</h1>
        </div>
