...
import { MovieBanner } from '../select-show/[id]/movie-banner';

export default function TheaterSeats() {
  // Initial theater layout (hardcoded)
  const initialSeats = [
    [1, 1, 1, 0, 1, 1, 'B', 1, 1, 0], // Row A
    [1, 'B', 1, 1, 0, 1, 1, 'B', 1, 1], // Row B
    [1, 1, 'B', 0, 1, 1, 1, 1, 0, 'B'], // Row C
    ['B', 1, 1, 1, 1, 0, 1, 1, 1, 1], // Row D
    [1, 1, 0, 'B', 1, 1, 0, 1, 'B', 1], // Row E
    [1, 0, 1, 1, 'B', 1, 1, 0, 1, 1], // Row F
    ['B', 1, 1, 0, 1, 'B', 1, 1, 0, 1], // Row G
    [1, 1, 'B', 1, 0, 1, 1, 'B', 1, 0]  // Row H
  ];
  ...
  const handleBooking = () => {
    ...
    selectedSeats.forEach(seat => {
      const rowLetter = seat.charCodeAt(0) - 65;
      const seatNumber = parseInt(seat.slice(1)) - 1;
      newSeats[rowLetter][seatNumber] = 'B';
    });
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto p-6 font-sans">
      <h2 className="text-2xl font-bold text-center mb-6">Movie Theater Seats</h2>
      
      <div className="flex justify-center gap-8 mt-30 mb-6">
            ...
            {row.map((seat, seatIndex) => (
              <button
                ...
                className={`w-7 h-7 rounded flex items-center justify-center text-xs transition-all
                  ${seat === 0 ? 'bg-gray-300 cursor-not-allowed' :
